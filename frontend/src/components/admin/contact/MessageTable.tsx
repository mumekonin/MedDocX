  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">Messages submitted through your contact form.</p>
      </div>
      ...
      {messages && messages.length > 0 && (
        <div className="space-y-2">
          {messages.map((message) => (
            ...
            >
              {message.isRead ? (
                <MailOpen className="w-4 h-4 text-gray-500 shrink-0" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className={`text-sm truncate ${message.isRead ? "text-gray-300" : "text-white font-semibold"}`}>
                    {message.name}
                  </p>
                  <span className="text-gray-600 text-xs shrink-0">{message.email}</span>
                </div>
                <p className={`text-sm truncate ${message.isRead ? "text-gray-500" : "text-gray-300"}`}>
                  {message.subject}
                </p>
              </div>
      ...
      {selectedMessage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
          ...
          <div className="relative bg-[#0d0d14] border border-white/10 rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto">
            ...
            <div className="p-7">
              <h3 className="text-white font-bold text-lg mb-1">{selectedMessage.subject}</h3>
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="w-11 h-11 shrink-0 rounded-full bg-white/5 hover:bg-red-500/10 flex items-center justify-center text-gray-400 hover:text-red-400 transition-colors"
                  aria-label="Delete"
                >
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center px-4">
          ...
          <div className="relative bg-[#0d0d14] border border-white/10 rounded-2xl w-full max-w-sm p-6">
            ...
            <p className="text-gray-400 text-sm mb-6">
              ...
              <span className="text-white">{deleteTarget.name}</span>? This can't be undone.
            </p>
            <div className="flex items-center gap-3">
