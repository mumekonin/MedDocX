...
const FaqTable = () => {
  ...
  return (
    <div>
      ...
      {faqs && faqs.length === 0 && <p className="text-gray-500 text-sm">No FAQs added yet.</p>}
      ...
      {faqs && faqs.length > 0 && (
        <div className="space-y-3">
          {faqs.map((faq) => (
            <div
              key={faq.id}
              className="bg-white/[0.03] border border-white/10 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-start gap-4"
            >
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
          ...
          <div className="relative bg-[#0d0d14] border border-white/10 rounded-2xl w-full max-w-sm p-6">
            <h3 className="text-white font-bold mb-2">Delete FAQ</h3>
